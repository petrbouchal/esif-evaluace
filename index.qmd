...
```{r load}
evals_all <- read_rds("data-interim/evals_for_table.rds")
```
  ...
  rtrn <- tagList(
    ...
    if(any(!is.na(dt$etapa_nazev))) {
      ...
      tags$div(
        ...
        eval_field("Datum dokončení: ", if(!is.na(dt$etapa_ukon_datum)) ptrr::format_date_human(dt$etapa_ukon_datum) else NA),
        ...
        tags$div(
          ...
          purrr::pmap(list(odkazy$file_url, odkazy$file_text, odkazy$link), function(x, y, link) 
            if(is.na(x)) NULL else tags$div(tags$a(y, href = x), 
